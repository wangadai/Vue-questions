...
- 同时出现的两种情况处理
    -   代码：（`v-for`遍历初始数据，`v-if`根据item数据判断）
        优化：过滤一个列表中的项目，在这种情形下，将初始数据替换为一个计算属性，让其返回过滤后的列表。
    -   代码：（`v-for`遍历初始数据，`v-if`根据初始数据判断）
...
- 功能区别：
    - `watch`更通用
    - `computed`派生功能都能实现，计算属性底层来自`watch`，但做的更多，比如缓存
- 用法区别
    - `computed`更简单高效，优先使用
    - 值变化与后端做交互则必须使用`watch`
- 使用场景
    - `watch`一条数据影响多条数据时
    - `computed`一个属性受多个属性影响时
...
1. 全局导航钩子： 
    ...
    - router.afterEach      （常用自动让页面返回最顶端）
        > 和守卫不同的是，这些钩子不会接受 next 函数也不会改变导航本身

2. 路由独享的导航钩子： beforeEnter
...
3. 组件内的钩子函数： 
    ...
    - beforeRouteLeave
        ...
        > 离开守卫beforeRouteLeave：通常用来禁止用户在还未保存修改前突然离开。该导航可以通过 next(false) 来取消：
                ...
                beforeRouteLeave (to, from , next) {
                    const answer = window.confirm('是否确定离开？')
                    if (answer) {
                        next()
                    } else {
                        next(false)
